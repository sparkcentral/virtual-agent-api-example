...
class RestClient:
    ...
    def send(self, conversation_id, text=None, topics=None, complete=None):
        """
        ...
        request_client = OAuth2Session(client=client)
        request_client.fetch_token(token_url=f'{self.base_url}/virtual-agent/oauth2/token',
                                   client_id=self.client_id,
                                   client_secret=self.client_secret,
                                   scope='client-read')
        return request_client.post(f'{self.base_url}/virtual-agent/conversations/{conversation_id}',
                                   json={
                                       'sendMessage': text and {'text': text},
                                       'applyTopics': topics,
                                       'complete': complete
                                   })
